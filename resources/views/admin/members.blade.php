...
<div x-data="{ 
    ...
    <!-- Títulos y Botón adaptativo -->
    <div class="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight mb-1">Control de Miembros</h1>
            <p class="text-xs sm:text-sm text-gray-400">Panel administrativo enlazado a MariaDB con funciones de gestión y cobro en mostrador.</p>
        </div>
        <button @click="initCreate()" class="w-full lg:w-auto bg-brand-neon hover:bg-[#b3e600] text-black font-black text-xs uppercase tracking-wider py-3 px-5 rounded-xl transition duration-200 shadow-lg shadow-brand-neon/10 text-center">
            + Registrar Nuevo Miembro
        </button>
    </div>

    <!-- Barra de Filtros responsiva -->
    <div class="bg-[#141414] border border-neutral-900 rounded-xl p-4 mb-6 flex flex-col sm:flex-row gap-4">
        <input x-model="searchQuery" @keydown.enter="window.location.href = '{{ route('admin.members') }}?search=' + searchQuery + '&status=' + statusFilter" type="text" class="bg-[#1c1c1c] border-transparent rounded-lg text-sm text-white px-4 py-2 w-full sm:w-72 focus:border-brand-neon focus:ring-0 placeholder-gray-500" placeholder="Buscar y presionar Enter...">
        <select x-model="statusFilter" @change="window.location.href = '{{ route('admin.members') }}?search=' + searchQuery + '&status=' + statusFilter" class="bg-[#1c1c1c] border-transparent rounded-lg text-sm text-white px-4 py-2 w-full sm:w-auto focus:border-brand-neon focus:ring-0">
    ...
    <!-- CONTENEDOR DE TABLA CON SCROLL HORIZONTAL PROTEGIDO -->
    <div class="bg-[#141414] border border-neutral-900 rounded-xl overflow-hidden w-full">
                ...
                <tbody class="divide-y divide-neutral-900 text-sm text-gray-300">
                    @forelse($members as $member)
                        <tr class="hover:bg-[#1a1a1a]/10 transition">
                            <td class="px-6 py-4 font-bold text-white whitespace-nowrap md:whitespace-normal">
                                {{ $member->name }} 
                                <span class="block text-[11px] text-gray-500 font-normal">{{ $member->email }}</span>
                            </td>
    ...
    <!-- MODAL DE FORMULARIO DE ALTA Y EDICIÓN COMPLETA (Protegido contra cortes verticales) -->
    <div x-show="openFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" x-cloak>
        <div @click.away="openFormModal = false" class="bg-[#141414] border border-neutral-800 w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
            <div class="px-6 py-4 border-b border-neutral-800 flex justify-between items-center bg-[#1c1c1c]/40 shrink-0">
                <h2 class="text-xs sm:text-sm font-black text-white uppercase tracking-wider" x-text="isEdit ? 'Editar Expediente y Membresía' : 'Matricular Atleta & Registrar Cobro'"></h2>
                <button @click="openFormModal = false" class="text-gray-400 hover:text-white text-xl focus:outline-none">&times;</button>
            </div>
            
            <form :action="formAction" method="POST" class="p-6 space-y-4 overflow-y-auto flex-1">
                @csrf
                ...
                <!-- 📱 Ajuste de las tarjetas de cobro a grid responsivo -->
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label class="flex items-center justify-between p-3 rounded-lg border bg-[#1c1c1c] cursor-pointer transition select-none" :class="metodoPago === 'efectivo' ? 'border-brand-neon text-white' : 'border-neutral-800 text-gray-400'">
                            ...
                            <div class="w-3.5 h-3.5 rounded-full border flex items-center justify-center shrink-0" :class="metodoPago === 'efectivo' ? 'border-brand-neon' : 'border-neutral-600'">
                        ...
                        <label class="flex items-center justify-between p-3 rounded-lg border bg-[#1c1c1c] cursor-pointer transition select-none" :class="metodoPago === 'tarjeta' ? 'border-brand-neon text-white' : 'border-neutral-800 text-gray-400'">
                            ...
                            <div class="w-3.5 h-3.5 rounded-full border flex items-center justify-center shrink-0" :class="metodoPago === 'tarjeta' ? 'border-brand-neon' : 'border-neutral-600'">
    ...
    <!-- FICHA DEL ATLETA MODAL RESPONSIVO -->
    <div x-show="openProfileModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" x-cloak>
        <div @click.away="openProfileModal = false" class="bg-[#141414] border border-neutral-800 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
            <div class="p-6 border-b border-neutral-800 flex justify-between items-start bg-[#1c1c1c]/50 shrink-0">
                <div class="flex items-center gap-4 min-w-0">
                    <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-brand-neon text-black font-black flex items-center justify-center text-sm shrink-0" x-text="activeMember.name ? activeMember.name.split(' ').map(n => n[0]).join('').substring(0,2) : ''"></div>
                    <div class="min-w-0">
                        <h3 class="text-sm sm:text-md font-bold text-white uppercase tracking-tight truncate" x-text="activeMember.name"></h3>
                        <span class="text-[10px] text-gray-500 block font-medium mt-0.5 truncate" x-text="'Matriculado en: ' + activeMember.joins"></span>
                    </div>
                </div>
                <button @click="openProfileModal = false" class="text-gray-400 hover:text-white text-xl focus:outline-none">&times;</button>
            </div>
            
            <div class="p-6 space-y-4 overflow-y-auto flex-1">
                <!-- 📱 Campos mutados a un grid apilable en celulares -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b border-neutral-800/60 pb-4">
                    <div>
                    ...
                    <div class="min-w-0">
                        ...
                        <span class="text-xs text-white font-medium break-all sm:truncate block" x-text="activeMember.email"></span>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 bg-[#1c1c1c]/30 border-t border-neutral-800 flex justify-between items-center shrink-0">
    ...
    <!-- MODAL DE ELIMINACIÓN RESPONSIVO -->
